...
namespace Invector.vCharacterController
{
    ...
    public class vThirdPersonMotor : vCharacter, vIAnimatorStateInfoController
    {
        ...
        {
            RaycastHit hit;
            Vector3 origin = transform.position + Vector3.up * colliderRadiusDefault * 0.5f;
            ...
            else if (checkStopMoveCondition && Physics.Raycast(origin, direction, out hit, distance, groundLayer))
            {
        ...
        protected virtual void SlideMovementBehavior()
        {
            ...
            if(debugWindow) Debug.DrawRay(transform.position, dir * slideDownVelocity);
            ...
            if (_rotateSlopeEnterTime <= 0f)
            {
                ...
                if (debugWindow) Debug.DrawRay(transform.position, rightMovement * slideSidewaysVelocity, Color.blue);
